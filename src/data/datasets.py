...
def _nullable_boolean(values: pd.Series, name: str) -> pd.Series:
    text_mapping = {"true": True, "false": False, "1": True, "0": False,
...
def validate_manifest(manifest: pd.DataFrame, allow_final_test: bool = False) -> pd.DataFrame:
    ...
    for column in ("lesion_present", "normal_skin", "localization_available", "supported_for_lesion_detection", "supported_for_lesion_presence", "supported_for_diagnosis", "other_skin_condition"):
